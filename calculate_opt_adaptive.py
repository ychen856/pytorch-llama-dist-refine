    ...
    for (start_idx, end_idx), records in all_data.items():
        ...
        for record in records:
            ...
            if (record["client_computation_time"] is not None and
                ...
                latency_comm += record["communication_time"]
                latency_server += record["server_computation_time"]
        ...
        WEIGHT_EARLY = exit_rate_manager.compute_weight(logger, early_rate, latency_client/(client_count + 1e-6), latency_comm/(comm_count + 1e-6), latency_server/(server_count + 1e-6))
